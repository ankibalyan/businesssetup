...
jQuery.fn.ajaxSubmit = function(options) {
	...
	var url = (typeof action === 'string') ? jQuery.trim(action) : '';
	if (url) {
   ...
   if (options.iframe !== false && (fileInputs || options.iframe || multipart)) {
	...
	function fileUpload() {
		...
		window[fn] = function() {
			var f = jQueryio.data('form-plugin-onload');
			if (f) {
				f();
				window[fn] = undefined;
		...
		var jQueryio = jQuery('<iframe id="' + id + '" name="' + id + '" src="'+ s.iframeSrc +'" onload="window[\'_\'+this.id]()" />');
		var io = jQueryio[0];
			...
			try {
				if (s.extraData) {
					for (var n in s.extraData) {
						extraInputs.push(
							jQuery('<input type="hidden" name="'+n+'" value="'+s.extraData[n]+'" />')
								.appendTo(form)[0]);
					}
				}
				...
				jQueryio.data('form-plugin-onload', cb);
				form.submit();
			}
			finally {
		...
		function cb() {
			...
			jQueryio.removeData('form-plugin-onload');
			...
			try {
				...
				ok = false;
				xhr.error = e;
				jQuery.handleError(s, xhr, 'error', e);
			}
