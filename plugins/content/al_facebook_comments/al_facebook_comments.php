...
class plgContental_facebook_comments extends JPlugin {
	...
	private function  getPlugInCommentsHTML($article, $url, $title) {
		...
		$tmp = "<fb:comments href=\"$url\" num_posts=\"$number_comments\" width=\"450\" colorscheme=\"$color_scheme\"></fb:comments>";
		...
		if($idioma == 'es'){
			...
			$link = 'http://www.alexlopezit.com/plugin-comentarios-de-facebook-para-joomla';
		}else{
			$al = 'Facebook Social Comments';
	...
	private function fb_share($title, $url, $summary, $image, $text){
		...
		$fbshare = "<a onClick=\"window.open('http://www.facebook.com/sharer.php?s=100&amp;p[title]=$title&amp;p[summary]=$summary&amp;p[url]=$url&amp;&p[images][0]=$image', 'sharer', 'toolbar=0,status=0,width=548,height=325');\" href=\"javascript: void(0)\"><i class=\"al_fbshare_icon\"></i>$text</a>";
		return $fbshare;
	}
