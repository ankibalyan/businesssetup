...
/**
 * Methods supporting a list of Publiclc records.
 */
class PubliclcModelRegistrationforms extends JModelList {
  ...
  //       $result = $db->loadObjectList();
    ...
    //                        $db->setQuery($strqry);
    //                        if($db->query())
    //                          $app->Redirect('index.php/public-limited-company-service-flow?params=2');
    //                        else
    ...
    //                                //die($strqry); 
    //                                  $db->setQuery($strqry);
    //                                  if($db->query()) {  $app->Redirect('index.php/public-limited-company-service-flow?params=2');
    //                                    } else
    //                                      $app->Redirect('index.php/service/incorporations/llp');
    //                          }                 
   ...
   function saveform1ContactInfo($service_flag) {
    ...
    for($i=1; $i <= $totalDir; $i++){
      ...
      if(isset($_POST["directorMail".$i])) $directorMail =$_POST["directorMail".$i];
      if(isset($_POST["directorEducation".$i])) $directorEducation =$_POST["directorEducation".$i];
      if(isset($_POST["directorPlace".$i])) $directorPlace =$_POST["directorPlace".$i];
     
      (isset($_POST["directordin".$i]["name"])) ? $directordin =$_FILES["directordin".$i]["name"] : $directordin ="";
      (isset($_POST["pancard".$i]["name"])) ? $pancard =$_FILES["pancard".$i]["name"] : $pancard ="";
    
      
  if (!isset($_FILES["photograph".$i]["error"]) || !$_FILES["photograph".$i]["error"] > 0) {
      ...
      (isset($_POST["addressproof".$i])) ? $addressproof =$_POST["addressproof".$i] : $addressproof="";
      (isset($_POST["idproof".$i])) ? $idproof =$_POST["idproof".$i] : $idproof = "";
      (isset($_POST["idprooffile".$i]["name"])) ? $idproof_file=$_FILES["idprooffile".$i]["name"] : $idproof_file="";
      (isset($_POST["addressprooffile".$i]["name"])) ? $addressproof_file=$_FILES["addressprooffile".$i]["name"] : $addressproof_file="";
     
      (isset($_POST["directorname".$i])) ? $directorname =$_POST["directorname".$i] : $directorname="";
      (isset($_POST["directorAddress".$i])) ? $directorAddress =$_POST["directorAddress".$i] : $directorAddress="";
      (isset($_POST["directorMail".$i])) ? $directorMail =$_POST["directorMail".$i] : $directorMail="";
      (isset($_POST["directorEducation".$i])) ? $directorEducation =$_POST["directorEducation".$i] : $directorEducation="";
      (isset($_POST["directorPlace".$i])) ? $directorPlace =$_POST["directorPlace".$i] : $directorPlace="";

      (isset($_POST["directordin".$i]["name"])) ? $directordin =$_FILES["directordin".$i]["name"] : $directordin="";
      (isset($_POST["pancard".$i]["name"])) ? $pancard =$_FILES["pancard".$i]["name"] : $pancard="";
      (isset($_POST["photograph".$i]["name"])) ? $photograph =$_FILES["photograph".$i]["name"] : $photograph="";
      if(! $promo_check){
          ...
          move_uploaded_file($_FILES["pancard".$i]["tmp_name"],$ftppath.'/'.$_FILES["pancard".$i]["name"]);
                   ...
                   if(isset($_POST["recId".$i])){
                            ...
                            if($addressproof_file !='')
                            {
                                ...
                                if($idproof !=''){
                                $idproof="dir_idproof='$addressproof',";
                                }
